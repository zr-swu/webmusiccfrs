...
<html>
  ...
  <body>
    <div class="col-lg-6" style="max-width: 100%;padding:0">
    	<form action="${pageContext.request.contextPath}/admin_playlist_list" method="post" id="form">
    		  <input type="hidden" name="pageBean.page" value="${pageBean.page}" id="page">
		      <div class="card">
		          <div class="card-header" style="padding: 0.53rem 1.25rem;">
		              <strong class="card-title">歌单列表</strong>
		              <div style="display:inline-block;padding-left:70px">	
			          	<input type="text" id="title" name="title" value="${title}" 
              	  			maxlength="20"  class="" style="display:inline-block;" placeholder="请输入歌单名" >	
		                  	...
		                  	<s:if test="pageBean!=null && pageBean.list!=null">
		                  		<s:iterator var="playlist" value="pageBean.list" status="status">
		                  			<tr>
			                      		...
			                      		<td><s:property value="#playlist.title"/></td>
			                      		<td>
			                      			<button type="button" class="self_btn" style="background-color:#2d6390"
			              					onclick="javascript:location.href='${pageContext.request.contextPath}/admin_playlist_view?playlistid=<s:property value="#playlist.id"/>'">详情</button>
			                      			&nbsp;&nbsp;
			                      			<button type="button" class="self_btn" style="background-color:#2d6390"
			              					onclick="javascript:location.href='${pageContext.request.contextPath}/admin_playlist_addOrUpdate?playlistid=<s:property value="#playlist.id"/>'">修改</button>
			                      			&nbsp;&nbsp;
			                      			<button type="button" class="self_btn" style="background-color:#2d6390"
			              					onclick="javascript:doDelete('${pageContext.request.contextPath}/admin_playlist_delete?playlistid=<s:property value="#playlist.id"/>')">删除</button>
			                      		</td>
			                      	</tr>
		                  		</s:iterator>
		                    </s:if>
