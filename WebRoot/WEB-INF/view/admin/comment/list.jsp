    	...
    	<form action="${pageContext.request.contextPath}/admin_score_list" method="post" id="form">
		      ...
		      <div class="card">
		          <div class="card-header" style="padding: 0.53rem 1.25rem;">
		              <strong class="card-title">歌单评论列表</strong>
		              ...
		              <table class="table table-bordered" style="text-align: center;">
		                  ...
		                  <tbody>
		                  	<s:if test="pageBean!=null && pageBean.list!=null">
		                  		<s:iterator var="comment" value="pageBean.list" status="status">
		                  			<tr>
			                      		<td><s:property value="#status.index+1"/></td>
			                      		<td><s:property value="#comment.user.username"/></td>
			                      		<td style="width:30%"><s:property value="#comment.playlist.title"/></td>
			                      		<td style="width:30%">
			                      			<div style="word-break:break-all;word-wrap:break-word;width:100%">
									  			<s:property value="#comment.comment"/>
									  		</div>
			                      		</td>
			                      		<td><s:property value="#comment.createtime"/></td>
			                      		<td>
			                      			<button type="button" class="self_btn" style="background-color:#2d6390"
			              					onclick="javascript:doDelete('${pageContext.request.contextPath}/admin_comment_delete?commentid=<s:property value="#comment.id"/>')">删除</button>
			                      		</td>
			                      	</tr>
		                  		</s:iterator>
		                    </s:if>
